...
-- Fixed version with proper table drop order and consistent ID naming
...
-- In a production environment, make sure to hash the password properly
...
SELECT 
    ...
    'Administrator',
    [RoleID],
    1, -- Default to first branch (Head Office)
    1,
    @passwordHash,
    @salt,
    GETDATE()
...
SELECT 
    ...
    0
FROM [dbo].[Users] u
CROSS JOIN [dbo].[Notifications] n
WHERE u.[Username] = 'admin' AND n.[Title] = 'Welcome to Oven Delights ERP';
