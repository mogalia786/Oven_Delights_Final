...
Public Class APInvoiceService
    ...
    Public Function GetCategories() As DataTable
        ...
        Using conn As New SqlConnection(_connectionString)
            conn.Open()
            ' Load all expense accounts (5xxx) from Chart of Accounts
            Dim sql As String = "
                SELECT 
                    AccountID AS CategoryID,
                    AccountName AS CategoryName,
                    AccountCode AS GLAccountCode
                FROM ChartOfAccounts
                WHERE IsActive = 1 
                    AND IsSubsidiaryLedger = 0
                    AND (AccountCode LIKE '5%' OR AccountCode LIKE '2%' OR AccountCode LIKE '1%')
    ...
    ''' ACCRUAL MODEL: Payment initiation does NOT post to GL
    ''' GL posting happens when bank statement confirms payment
    ''' </summary>
    Public Function PostSinglePaymentToGL(invoiceId As Integer,
        ...
        ' ACCRUAL ACCOUNTING: Do NOT post to GL when payment is initiated
        ' The bank statement will complete the double-entry when payment clears
        ' Invoice already posted: DR Expense / CR AP
        ' Bank statement will post: DR AP / CR Bank (completes double-entry)
        
        ' Return 0 to indicate no GL posting occurred (this is correct behavior)
        Return 0
    End Function

    ''' <summary>
    ''' Post batch payment to General Ledger
    ''' ACCRUAL MODEL: Batch payment initiation does NOT post to GL
    ''' GL posting happens when bank statement confirms payment
    ''' </summary>
    Public Sub PostBatchPaymentToGL(batchId As Integer,
                                    paymentDate As Date,
                                    createdBy As String)
        ' ACCRUAL ACCOUNTING: Do NOT post to GL when batch payment is initiated
        ' The bank statement will complete the double-entry when payments clear
        ' Invoices already posted: DR Expense / CR AP
        ' Bank statement will post: DR AP / CR Bank (completes double-entry)
        
        ' No GL posting - batch payment just sends instructions to bank
    End Sub
