...
Public Class APPaymentService
    ...
    Public Function SubmitPaymentBatchToFNB(batchId As Integer, Optional paymentDate As Date? = Nothing, Optional batchPayment As Boolean = False) As Boolean
        Try
            ...
            Dim paymentRequestJson = BuildFNBPaymentRequest(batchData, paymentDate, batchPayment)

            ' Save request JSON
            UpdateBatchRequestJSON(batchId, paymentRequestJson)

            RaiseEvent LogMessage($"Submitting batch to FNB API...")
            RaiseEvent LogMessage($"Payment Request JSON:{Environment.NewLine}{paymentRequestJson}")
            ...
            RaiseEvent LogMessage($"Calling FNB API InitiatePayment...")
            Dim response = _fnbApiClient.InitiatePayment(paymentRequest)
            RaiseEvent LogMessage($"FNB API call completed")
    ...
    Private Function BuildFNBPaymentRequest(batchData As DataTable, Optional paymentDate As Date? = Nothing, Optional batchPayment As Boolean = False) As String
        Dim transactions As New List(Of CreditTransferTransaction)

        For Each row As DataRow In batchData.Rows
            transactions.Add(New CreditTransferTransaction With {
                .endToEndId = row("InvoiceNumber").ToString(),
                .amount = New Amount With {
                    .currency = "ZAR",
                    .value = CDec(row("TotalAmount"))
                },
                ...
                .creditorAccount = New CreditorAccount With {
                    ...
                    .accountType = If(row("AccountType").ToString() = "Cheque", "CACC", "SVGS")
                },
                .creditorAgent = New CreditorAgent With {
                    .branchId = row("BranchCode").ToString()
                },
                .remittanceInformationUnstructured = row("Description").ToString(),
                .remittanceLocationMethod = "EMAL",
                .remittanceLocationElectronicAddress = "Tshepo.Kgasoane@rmb.co.za"
            })
        Next

        Dim totalAmount = batchData.AsEnumerable().Sum(Function(r) CDec(r("TotalAmount")))
        Dim messageId = "APB-" & DateTime.Now.ToString("yyyyMMddHHmmss")
        Dim paymentInfoId = "APPI-" & DateTime.Now.ToString("yyyyMMddHHmmss")

        Dim paymentRequest = New PaymentInitiationRequest With {
            .groupHeader = New GroupHeader With {
                .messageId = messageId,
            ...
            .paymentInformation = New List(Of PaymentInformation) From {
                New PaymentInformation With {
                    .paymentInformationId = paymentInfoId,
                    .paymentInformationMethod = "TRF",
                    .batchBooking = batchPayment,
                    .numberOfTransactions = transactions.Count,
                    .controlSum = totalAmount,
                    .paymentTypeInformationServiceLevelCode = "NURG",
                    .requestedExecutionDate = If(paymentDate, DateTime.Now).ToString("yyyy-MM-dd"),
                    .debtor = New Debtor With {
                        .name = "Oven Delights",
                        .bicOrBEI = ""
                    },
                    .debtorAccount = New DebtorAccount With {
                        .accountNumber = GetDebtorAccountNumber(),
                        .accountType = "CACC"
                    },
        ...
        Return JsonConvert.SerializeObject(paymentRequest, New JsonSerializerSettings With {
    ...
    Private Sub PostBatchToGL(batchId As Integer)
        Try
            RaiseEvent LogMessage($"Batch {batchId} payment completed - GL posting will occur when bank statement is imported")
        ...
        Catch ex As Exception
            RaiseEvent LogMessage($"Error updating batch status: {ex.Message}")
        End Try
    End Sub
