...
Public Class FNBStatementService
    ...
    Public Function SaveStatementToDatabase(statement As CustomerStatement, fetchedBy As String) As Integer
        ...
        Try
            If statement Is Nothing OrElse statement.statement Is Nothing Then
                Return 0
            End If
            ...
            Using conn As New SqlConnection(_connectionString)
                ...
                If statement.statement.balance IsNot Nothing AndAlso statement.statement.balance.Count > 0 Then
                    For Each bal In statement.statement.balance
                        Try
                            Dim cmdBalance As New SqlCommand("
                                INSERT INTO AP_StatementBalances (AccountNumber, BalanceType, Amount, Currency, CreditDebitIndicator, BalanceDate, FetchedBy, FetchedDate)
                                VALUES (@AccountNumber, @BalanceType, @Amount, @Currency, @CreditDebitIndicator, @BalanceDate, @FetchedBy, GETDATE())", conn)
                            
                            cmdBalance.Parameters.AddWithValue("@AccountNumber", If(statement.statement.account IsNot Nothing, statement.statement.account.accountNumber, ""))
                            cmdBalance.Parameters.AddWithValue("@BalanceType", If(String.IsNullOrEmpty(bal.typeCode), DBNull.Value, bal.typeCode))
                            cmdBalance.Parameters.AddWithValue("@Amount", bal.amountValue)
                            cmdBalance.Parameters.AddWithValue("@Currency", If(String.IsNullOrEmpty(bal.amountCurrency), "ZAR", bal.amountCurrency))
                            cmdBalance.Parameters.AddWithValue("@CreditDebitIndicator", If(String.IsNullOrEmpty(bal.creditDebitIndicator), DBNull.Value, bal.creditDebitIndicator))
                            cmdBalance.Parameters.AddWithValue("@BalanceDate", If(String.IsNullOrEmpty(bal.date), DBNull.Value, DateTime.Parse(bal.date)))
                            cmdBalance.Parameters.AddWithValue("@FetchedBy", fetchedBy)
                            
                            cmdBalance.ExecuteNonQuery()
                            RaiseEvent LogMessage($"Saved balance: {bal.typeCode} - {bal.creditDebitIndicator} R{bal.amountValue:N2} on {bal.date}")
                        Catch ex As Exception
                            RaiseEvent LogMessage($"Error saving balance: {ex.Message}")
                        End Try
                    Next
                End If

                ' Save transaction entries
                If statement.statement.entry Is Nothing Then
                    Return 0
                End If
