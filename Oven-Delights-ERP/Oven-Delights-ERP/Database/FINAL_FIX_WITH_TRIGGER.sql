-- FINAL FIX - Disable trigger during product creation to avoid NULL issues
...
BEGIN
    ...
    BEGIN TRY
        ...
        DISABLE TRIGGER trg_AfterUpdateRecipeCosts_OnProductPriceChange ON Demo_Retail_Product;
        
        -- Step 1: Check if product already exists
        IF EXISTS (SELECT 1 FROM Demo_Retail_Product WHERE Name = @ProductName AND SKU = @SKU)
        BEGIN
            ENABLE TRIGGER trg_AfterUpdateRecipeCosts_OnProductPriceChange ON Demo_Retail_Product;
...
-- Test the procedure
PRINT 'Testing product creation with trigger disabled...'
GO

DECLARE @TestProductID INT;

EXEC sp_SaveProductToAllBranches
    @ProductName = 'TEST TRIGGER FIX',
    @ProductCode = 'TESTTRIGGER',
    @SKU = 'SKU-TRIGGER-001',
    @CategoryID = 22,
    @SubcategoryID = 18,
    @ItemType = 'internal',
...
WHERE Name = 'TEST TRIGGER FIX'
ORDER BY BranchID;

SELECT 'Demo_Retail_Variant' AS TableName, VariantID, ProductID, AttributesJson, IsDefault
FROM Demo_Retail_Variant
WHERE ProductID IN (SELECT ProductID FROM Demo_Retail_Product WHERE Name = 'TEST TRIGGER FIX')
ORDER BY VariantID;
...
WHERE v.ProductID IN (SELECT ProductID FROM Demo_Retail_Product WHERE Name = 'TEST TRIGGER FIX')
ORDER BY s.BranchID;

SELECT 'Demo_Retail_Price' AS TableName, pr.ProductID, pr.BranchID, pr.SellingPrice, pr.CostPrice
FROM Demo_Retail_Price pr
WHERE pr.ProductID IN (SELECT ProductID FROM Demo_Retail_Product WHERE Name = 'TEST TRIGGER FIX')
ORDER BY pr.BranchID;

PRINT ''
PRINT 'SUCCESS! If you see TEST TRIGGER FIX in all 4 tables for each branch, the fix is complete!'
PRINT 'The trigger is now re-enabled and will work normally for updates.'
