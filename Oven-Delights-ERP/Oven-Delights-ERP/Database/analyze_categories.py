...
csv_path = r"C:\Development Apps\Cascades projects\Oven-Delights-ERP\Oven-Delights-ERP\Oven-Delights-ERP\Database\Copy of ITEM LIST NEW 2025 updated.csv"
...
print(f"\n{'='*80}")
print(f"MAIN CATEGORIES BREAKDOWN")
print(f"{'='*80}\n")
...
print(f"\n{'='*80}")
print(f"ITEM CATEGORY (internal/external) DISTRIBUTION")
print(f"{'='*80}\n")
...
print(f"\n{'='*80}")
print(f"POS VISIBILITY RECOMMENDATION")
print(f"{'='*80}\n")
...
for main_cat in sorted(categories.keys()):
    ...
    if has_internal and not has_external:
        visibility = "POS: YES (internal only - manufactured)"
    elif has_external and not has_internal:
        visibility = "POS: YES (external only - purchased for resale)"
    elif has_internal and has_external:
        visibility = "POS: PARTIAL (filter by external for resale, internal for manufactured)"
    else:
        visibility = "POS: UNKNOWN"
    
    print(f"{main_cat:30} | Types: {types:20} | {visibility}")
