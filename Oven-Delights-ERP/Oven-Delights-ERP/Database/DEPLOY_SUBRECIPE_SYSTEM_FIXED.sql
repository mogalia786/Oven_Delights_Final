-- =============================================
-- COMPLETE DEPLOYMENT SCRIPT - FIXED VERSION
-- Prepared Sub-Recipe Inventory System
-- Checks for existing tables and creates only what's needed
-- =============================================
...
-- Check if Demo_SubRecipe_Master exists
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'Demo_SubRecipe_Master')
BEGIN
    PRINT '  ERROR: Demo_SubRecipe_Master table does not exist!'
    PRINT '  Please run FIX_REORDER_BOOK_CREATE_RECIPE_TABLES.sql first'
    RAISERROR('Missing prerequisite table: Demo_SubRecipe_Master', 16, 1)
    RETURN
END
ELSE
BEGIN
    PRINT '  ✓ Demo_SubRecipe_Master exists'
END

-- Check if ReOrderBooks exists
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'ReOrderBooks')
BEGIN
    PRINT '  WARNING: ReOrderBooks table does not exist'
    PRINT '  Creating ReOrderBooks and ReOrderBookLines tables...'
    ...
    CREATE INDEX IX_ReOrderBooks_Manufacturer ON ReOrderBooks(ManufacturerID, Status)
    
    PRINT '  ✓ ReOrderBooks created'
END
ELSE
BEGIN
    PRINT '  ✓ ReOrderBooks exists'
END

-- Check if ReOrderBookLines exists
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'ReOrderBookLines')
BEGIN
    PRINT '  Creating ReOrderBookLines table...'
    ...
    PRINT '  ✓ ReOrderBookLines created'
END
ELSE
BEGIN
    PRINT '  ✓ ReOrderBookLines exists'
    ...
    BEGIN
        PRINT '  Adding ItemType column to ReOrderBookLines...'
...
-- =============================================
PRINT 'STEP 2: Creating inventory tables...'
...
-- =============================================
-- SP 3: Consume Sub-Recipe from Inventory (FIFO)
-- =============================================
