    ...
    Private Sub LoadProducts(Optional searchText As String = "")
        Try
            Using conn As New SqlConnection(connectionString)
                ...
                ' ALWAYS read recipes from Branch 6 (master branch) for ALL branches
                ' This makes recipes universal - all branches see the same products with recipes
                sql = "SELECT p.ProductID, p.Name AS ProductName, ISNULL(p.Code, p.SKU) AS SKU " & _
                      "FROM Demo_ProductRecipe_Master rm " & _
                      "INNER JOIN Demo_Retail_Product p ON p.ProductID = rm.ProductID " & _
                      "WHERE rm.IsActive = 1 " & _
                      "  AND p.IsActive = 1 " & _
                      "  AND p.BranchID = 6 " & _
