    ...
    Public Class AddProductForm
        ...
        Private Function IsProductCodeExists(productCode As String) As Boolean
            Try
                Using con As New SqlConnection(_connectionString)
                    con.Open()
                    Dim sql As String = "SELECT COUNT(DISTINCT ProductID) FROM Demo_Retail_Product WHERE Code = @Code"
        ...
        Private Function IsSKUExists(sku As String) As Boolean
            Try
                Using con As New SqlConnection(_connectionString)
                    con.Open()
                    Dim sql As String = "SELECT COUNT(DISTINCT ProductID) FROM Demo_Retail_Product WHERE SKU = @SKU"
                    Using cmd As New SqlCommand(sql, con)
        ...
        Private Sub SaveProduct()
            Using con As New SqlConnection(_connectionString)
                ...
                Try
                    ...
                    Dim categoryId As Integer = categorySelector.SelectedCategoryId.GetValueOrDefault(0)
                    Dim subcategoryId As Integer = categorySelector.SelectedSubcategoryId.GetValueOrDefault(0)
                    ...
                    Using cmd As New SqlCommand("sp_SaveProductToAllBranches", con)
                        ...
                        Dim subcatParam As New SqlParameter("@SubcategoryID", SqlDbType.Int)
                        subcatParam.Value = If(subcategoryId > 0, CType(subcategoryId, Object), DBNull.Value)
