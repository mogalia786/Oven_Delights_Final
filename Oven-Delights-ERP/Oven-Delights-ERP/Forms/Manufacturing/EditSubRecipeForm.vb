...
Public Class EditSubRecipeForm
    ...
    Private Sub btnAddIngredient_Click(sender As Object, e As EventArgs) Handles btnAddIngredient.Click
        ...
        ' Get ingredient details from Demo_Retail_Product (BranchID 6 = Master Products)
        Using conn As New SqlConnection(_connectionString)
            Dim query As String = "SELECT TOP 1 AverageCost, LastPaidPrice FROM Demo_Retail_Product WHERE ProductID = @ProductID AND BranchID = 6"
            Using cmd As New SqlCommand(query, conn)
            ...
            Using conn As New SqlConnection(_connectionString)
                ...
                Using trans = conn.BeginTransaction()
                    Try
                        ...
                        ' Delete from CORRECT table Demo_SubRecipe_Ingredients
                        ...
                        ' Insert into CORRECT table Demo_SubRecipe_Ingredients with all required fields
                        For Each row As DataGridViewRow In dgvIngredients.Rows
                            Dim quantity As Decimal = CDec(row.Cells("Quantity").Value)
                            Dim costPerUnit As Decimal = CDec(row.Cells("CostPerUnit").Value)
                            Dim totalLineCost As Decimal = quantity * costPerUnit
                            
                            Dim insertIngredient As String = "INSERT INTO Demo_SubRecipe_Ingredients (SubRecipeID, IngredientID, Quantity, UnitOfMeasure, PackageSize, CostPerUnit, IsActive, CreatedDate) VALUES (@SubRecipeID, @IngredientID, @Quantity, @Unit, @PackageSize, @CostPerUnit, 1, GETDATE())"
                            Using cmd As New SqlCommand(insertIngredient, conn, trans)
                                ...
                                cmd.Parameters.AddWithValue("@PackageSize", If(row.Cells("PackageSize").Value IsNot Nothing, row.Cells("PackageSize").Value, 1D))
