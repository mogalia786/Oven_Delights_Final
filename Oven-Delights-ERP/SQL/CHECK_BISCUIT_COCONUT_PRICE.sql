-- Check Biscuit Coconut pricing after invoice capture

-- 1. Find the product
SELECT ProductID, ProductName, BranchID 
FROM dbo.Demo_Retail_Product 
WHERE ProductName LIKE '%Biscuit Coconut%';
...
WHERE icl.ProductID IN (SELECT ProductID FROM dbo.Demo_Retail_Product WHERE ProductName LIKE '%Biscuit Coconut%')
ORDER BY ic.CreatedDate DESC;

-- 4. If no data, check what products exist
SELECT TOP 10 ProductID, ProductName, BranchID FROM dbo.Demo_Retail_Product;
