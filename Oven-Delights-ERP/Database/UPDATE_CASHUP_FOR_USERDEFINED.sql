-- Update Cash Up Stored Procedure to Include User Defined Orders
-- User Defined Orders are paid in full upfront and should be included in cash-up totals
...
BEGIN
    ...
    -- Includes SALE, ORDER (deposits), OrderCollection, and User Defined Orders
    ...
    SELECT 
        ...
        -- Payment breakdown (all types including orders and user defined)
        ...
        -- User Defined Orders (full payment upfront)
        ...
        -- User Defined Orders by payment method
        SUM(CASE WHEN ds.SaleType = 'UserDefined' AND ds.PaymentMethod = 'Cash' THEN ds.TotalAmount ELSE 0 END) AS UserDefinedCash,
        SUM(CASE WHEN ds.SaleType = 'UserDefined' AND ds.PaymentMethod = 'Card' THEN ds.TotalAmount ELSE 0 END) AS UserDefinedCard,
        SUM(CASE WHEN ds.SaleType = 'UserDefined' AND ds.PaymentMethod = 'Split' THEN ds.TotalAmount ELSE 0 END) AS UserDefinedSplit,
...
PRINT 'sp_GetEndOfDayCashUp stored procedure updated successfully'
...
PRINT '  - UserDefined transactions (user defined orders - full payment)'
GO
