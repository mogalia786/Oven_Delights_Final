-- =============================================
-- COMPLETE INVENTORY FLOW DEPLOYMENT - FIXED
...
-- =============================================
-- STEP 2: Fix sp_StartReOrderBook
-- =============================================
PRINT 'STEP 2: Fixing sp_StartReOrderBook...'
GO
...
-- =============================================
-- STEP 3: Create sp_ConsumeIngredientsFromManufacturing
-- =============================================
PRINT 'STEP 3: Creating sp_ConsumeIngredientsFromManufacturing...'
GO
...
BEGIN
    SET NOCOUNT ON;
        ...
        SELECT 
            ...
            (bl.Quantity / bh.BatchSize) * @QuantityProduced AS QuantityNeeded,
        ...
        BEGIN
            ...
            BEGIN
                ...
                SET @ErrorMsg = 'Insufficient stock for ingredient: ' + @IngredientName + 
                               '. Required: ' + CAST(@QuantityNeeded AS NVARCHAR(20)) + 
                               ', Available: ' + CAST(ISNULL(@CurrentStock, 0) AS NVARCHAR(20));
...
-- =============================================
-- STEP 4: Update sp_ConsumeSubRecipeFromInventory
-- =============================================
PRINT 'STEP 4: Updating sp_ConsumeSubRecipeFromInventory...'
GO
...
BEGIN
    SET NOCOUNT ON;
        ...
        BEGIN
            ...
            SELECT 
                bl.ItemID, p.Name, bl.Quantity, (bl.Quantity / bh.BatchSize) * @QuantityNeeded
            FROM 
                BOM_Lines bl
                INNER JOIN BOM_Header bh ON bl.BOMID = bh.BOMID
                INNER JOIN Demo_Retail_Product p ON bl.ItemID = p.ProductID
        ...
        BEGIN
            ...
            BEGIN
                ...
                DECLARE @SubRecipeErrorMsg NVARCHAR(500);
                SET @SubRecipeErrorMsg = 'Insufficient sub-recipe inventory for ' + @CurrentSubRecipeName + 
                                        '. Required: ' + CAST(@CurrentQuantityNeeded AS NVARCHAR(20)) + 
                                        ', Available: ' + CAST(@QuantityConsumed AS NVARCHAR(20));
                RAISERROR(@SubRecipeErrorMsg, 16, 1);
                RETURN;
            END
...
-- =============================================
-- STEP 5: Update sp_GetSubRecipeInventoryReport
-- =============================================
PRINT 'STEP 5: Updating sp_GetSubRecipeInventoryReport...'
GO
...
PRINT '  2. Fixed sp_StartReOrderBook - status = Pending'
PRINT '  3. Created sp_ConsumeIngredientsFromManufacturing'
PRINT '  4. Updated sp_ConsumeSubRecipeFromInventory - handles all BOM sub-recipes'
PRINT '  5. Updated sp_GetSubRecipeInventoryReport - shows Baker name'
PRINT ''
