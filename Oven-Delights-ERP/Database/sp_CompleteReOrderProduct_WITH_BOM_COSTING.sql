...
BEGIN
    ...
    BEGIN TRY
        ...
        -- =============================================
        -- CALCULATE COST OF SALES FROM BOM REQUISITION
        -- =============================================
        SELECT @TotalBOMCost = ISNULL(SUM(TotalCost), 0)
        FROM ReOrderBOMRequisition
        WHERE ReOrderLineID = @ReOrderLineID;
        
        -- Calculate unit cost: Total BOM Cost / Quantity Ordered
        IF @QuantityOrdered > 0
            SET @UnitCost = @TotalBOMCost / @QuantityOrdered;
        ELSE
            SET @UnitCost = 0;
        ...
        -- Update Demo_Retail_Price with manufacturing cost (if table exists)
        IF EXISTS (SELECT * FROM sys.tables WHERE name = 'Demo_Retail_Price')
        BEGIN
            -- Update or insert cost price
            ...
            ELSE
            BEGIN
                INSERT INTO Demo_Retail_Price (ProductID, BranchID, CostPrice, SellingPrice, EffectiveFrom, LastUpdated)
                VALUES (@ProductID, @BranchID, @UnitCost, 0, GETDATE(), GETDATE());
            END
        END
