-- Retail Master Setup Script (INLINE, no SQLCMD :r required)
-- Runs Phase 0 end-to-end. Safe to re-run.
-- Timestamp: 11-Sep-2025 21:45 SAST
...
BEGIN
    ...
    CREATE INDEX IX_Retail_Price_Product ON dbo.Retail_Price(ProductID, EffectiveFrom);
END
GO
...
BEGIN
    CREATE TABLE dbo.Retail_Stock(
        ...
        CONSTRAINT UQ_Retail_Stock UNIQUE(VariantID, LocationKey)
    );
END
GO
...
BEGIN
    ...
    CREATE INDEX IX_Retail_StockMovements_Variant ON dbo.Retail_StockMovements(VariantID, CreatedAt);
END
GO
