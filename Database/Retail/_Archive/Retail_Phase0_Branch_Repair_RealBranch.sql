...
/* Retail Phase 0 (Branch-aware) REAL BRANCH repair
   - Drops views first
   - Ensures a REAL INT column BranchID exists (no computed alias)
   - If legacy BranchId exists, copies its data into BranchID
...
/* 2) Ensure REAL BranchID INT exists and copy from legacy BranchId if present */
-- Retail_Price
...
IF COL_LENGTH('dbo.Retail_Price','BranchId') IS NOT NULL
    UPDATE p SET BranchID = COALESCE(BranchID, BranchId) FROM dbo.Retail_Price p WHERE (p.BranchID IS NULL) AND (p.BranchId IS NOT NULL);
...
IF COL_LENGTH('dbo.Retail_Stock','BranchId') IS NOT NULL
    UPDATE s SET BranchID = COALESCE(BranchID, BranchId) FROM dbo.Retail_Stock s WHERE (s.BranchID IS NULL) AND (s.BranchId IS NOT NULL);
...
IF COL_LENGTH('dbo.Retail_StockMovements','BranchId') IS NOT NULL
    UPDATE m SET BranchID = COALESCE(BranchID, BranchId) FROM dbo.Retail_StockMovements m WHERE (m.BranchID IS NULL) AND (m.BranchId IS NOT NULL);
...
-- Drop existing BranchKey if it exists and then recreate against REAL BranchID
