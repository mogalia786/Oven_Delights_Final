...
/* Retail Phase 0 (Branch-aware) SAFE DYNAMIC repair
   - Drops views first
   - Ensures a REAL INT BranchID exists (no computed alias)
   - Uses dynamic SQL for any statement that references conditional columns
   - Rebuilds indexes/constraints without expressions
   - Recreates views with BranchID
*/
...
/* 2) Ensure REAL BranchID INT exists and copy from legacy BranchId using dynamic SQL */
DECLARE @sql NVARCHAR(MAX);

-- Retail_Price: add BranchID if missing
IF COL_LENGTH('dbo.Retail_Price','BranchID') IS NULL
    ALTER TABLE dbo.Retail_Price ADD BranchID INT NULL;
-- copy from BranchId if it exists
...
-- Retail_Stock: add BranchID if missing, then copy from BranchId if exists
...
-- Retail_StockMovements: add BranchID if missing, then copy from BranchId if exists
...
/* 3) Computed keys for Retail_Stock */
