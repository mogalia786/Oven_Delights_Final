-- Execute All Database Scripts for Oven Delights ERP
-- Run this script to create all SARS compliance tables and triggers
...
PRINT '';
PRINT 'Creating product synchronization triggers and procedures...';
...
BEGIN
    ...
    USING (
        SELECT MaterialID, QuantityOnHand, LastUpdated
        FROM dbo.Inventory
        WHERE IsActive = 1
...
IF EXISTS (SELECT * FROM sys.triggers WHERE name = 'tr_GRN_AfterInsertUpdate')
    DROP TRIGGER tr_GRN_AfterInsertUpdate;

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'GoodsReceivedNotes')
BEGIN
    EXEC('
    CREATE TRIGGER tr_GRN_AfterInsertUpdate
    ...
    END
    ');
    PRINT 'Created GoodsReceivedNotes sync trigger';
END
ELSE
    PRINT 'GoodsReceivedNotes table not found - skipping trigger creation';
...
IF EXISTS (SELECT * FROM sys.triggers WHERE name = 'tr_Inventory_AfterUpdate')
    DROP TRIGGER tr_Inventory_AfterUpdate;

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'Inventory')
BEGIN
    EXEC('
    CREATE TRIGGER tr_Inventory_AfterUpdate
    ON dbo.Inventory
    AFTER UPDATE
    AS
    BEGIN
    ...
    END
    ');
    PRINT 'Created Inventory sync trigger';
END
ELSE
    PRINT 'Inventory table not found - skipping trigger creation';
...
IF EXISTS (SELECT * FROM sys.triggers WHERE name = 'tr_RawMaterials_AfterInsertUpdate')
    DROP TRIGGER tr_RawMaterials_AfterInsertUpdate;

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'RawMaterials')
BEGIN
    EXEC('
    CREATE TRIGGER tr_RawMaterials_AfterInsertUpdate
    ON dbo.RawMaterials
    AFTER INSERT, UPDATE
    AS
    BEGIN
    ...
    END
    ');
    PRINT 'Created RawMaterials sync trigger';
END
ELSE
    PRINT 'RawMaterials table not found - skipping trigger creation';
...
PRINT 'All SARS compliance tables and product sync triggers have been created.';
