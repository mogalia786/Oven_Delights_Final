...
Public Class InvoiceGRVForm
    ...
    Private Function CreateGRV(conn As SqlConnection, trans As SqlTransaction) As Integer
        Dim cmd As New SqlCommand("INSERT INTO Stockroom_GRV (SupplierID, POID, ReceivedDate, DeliveryNote, SubTotal, VAT, Total, CreatedBy, CreatedDate) OUTPUT INSERTED.GRVID VALUES (@SupplierID, @POID, @ReceivedDate, @DeliveryNote, @SubTotal, @VAT, @Total, @CreatedBy, @CreatedDate)", conn, trans)
        ...
        For Each row As DataGridViewRow In dgvLines.Rows
            If Convert.ToDecimal(row.Cells("ReceivedQty").Value) > 0 Then
                Dim lineCmd As New SqlCommand("INSERT INTO Stockroom_GRVLines (GRVID, ProductID, OrderedQty, ReceivedQty, UnitCost, LineTotal) VALUES (@GRVID, @ProductID, @OrderedQty, @ReceivedQty, @UnitCost, @LineTotal)", conn, trans)
    ...
    Private Function CreateInvoice(conn As SqlConnection, trans As SqlTransaction) As Integer
        Dim cmd As New SqlCommand("INSERT INTO Stockroom_Invoices (SupplierID, InvoiceNumber, InvoiceDate, DueDate, SubTotal, VAT, Total, Status, CreatedBy, CreatedDate) OUTPUT INSERTED.InvoiceID VALUES (@SupplierID, @InvoiceNumber, @InvoiceDate, @DueDate, @SubTotal, @VAT, @Total, @Status, @CreatedBy, @CreatedDate)", conn, trans)
        ...
        For Each row As DataGridViewRow In dgvLines.Rows
            ...
            If receivedQty > 0 Then
                ...
                If productType = "External" Then
                    ' Update Retail_Product stock
                    Dim cmd As New SqlCommand("UPDATE Retail_Product SET StockLevel = ISNULL(StockLevel, 0) + @Qty WHERE ProductID = @ProductID", conn, trans)
                    cmd.Parameters.AddWithValue("@Qty", receivedQty)
                    cmd.Parameters.AddWithValue("@ProductID", productId)
                    cmd.ExecuteNonQuery()
                ElseIf productType = "Internal" Then
                    ' Update RawMaterials stock
                    Dim cmd As New SqlCommand("UPDATE RawMaterials SET StockLevel = ISNULL(StockLevel, 0) + @Qty WHERE MaterialID = @MaterialID", conn, trans)
                    cmd.Parameters.AddWithValue("@Qty", receivedQty)
    ...
    Private Sub UpdateSupplierLedger(conn As SqlConnection, trans As SqlTransaction, invoiceId As Integer)
        Try
            ...
            Dim balCmd As New SqlCommand("UPDATE Stockroom_Suppliers SET Balance = ISNULL(Balance, 0) + @Amount WHERE SupplierID = @SupplierID", conn, trans)
