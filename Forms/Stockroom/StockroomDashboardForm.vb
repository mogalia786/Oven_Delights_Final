        ...
        Private Sub LoadFulfilledGrid()
            ...
            Try
                ...
                Using cn As New SqlConnection(cs)
                    ...
                    End Using
                    If stockLoc = 0 OrElse mfgLoc = 0 Then Return
                    Using cmd As New SqlCommand("SELECT IOH.InternalOrderID, IOH.InternalOrderNo, ISNULL(bts.ManufacturerName,N'Unassigned') AS Manufacturer, bts.Status " & _
                                              "FROM dbo.InternalOrderHeader IOH " & _
                                              "JOIN dbo.BomTaskStatus bts ON bts.InternalOrderID = IOH.InternalOrderID " & _
                                              "WHERE IOH.FromLocationID=@fromLoc AND IOH.ToLocationID=@toLoc " & _
                                              "  AND IOH.Status = N'Completed' " & _
                                              "  AND bts.Status = N'Pending' " & _
                                              "ORDER BY IOH.InternalOrderID DESC;", cn)
                        cmd.Parameters.AddWithValue("@fromLoc", stockLoc)
                        cmd.Parameters.AddWithValue("@toLoc", mfgLoc)
                        Using r = cmd.ExecuteReader()
