...
Public Partial Class POSForm
    ...
    Private Sub UpdateStock(conn As SqlConnection, trans As SqlTransaction, cartRow As DataRow)
        Dim sql As String = "UPDATE dbo.Retail_Stock SET QtyOnHand = QtyOnHand - @qty " &
        ...
        Dim moveSql As String = "INSERT INTO dbo.Retail_StockMovements (ProductID, BranchID, MovementType, Quantity, Reason, MovementDate, UserID) " &
                               "VALUES (@pid, @bid, 'Sale', @qty, @reason, @date, @uid)"
        
        Using cmd As New SqlCommand(moveSql, conn, trans)
            cmd.Parameters.AddWithValue("@pid", cartRow("ProductID"))
            cmd.Parameters.AddWithValue("@bid", _currentBranchId)
            cmd.Parameters.AddWithValue("@qty", -Convert.ToInt32(cartRow("Quantity")))
            cmd.Parameters.AddWithValue("@reason", $"POS Sale - {cartRow("SKU")}")
            cmd.Parameters.AddWithValue("@date", DateTime.Now)
            cmd.Parameters.AddWithValue("@uid", If(AppSession.CurrentUser?.UserID, 0))
            cmd.ExecuteNonQuery()
        End Using
