...
Public Class RetailInventoryAdjustmentForm
    ...
    Private Sub LoadProducts()
        Try
            ...
            Using conn As New SqlConnection(GetConnectionString())
                conn.Open()
                Dim cmd As New SqlCommand("SELECT ProductID, ProductCode, ProductName FROM RetailInventory WHERE IsActive = 1 ORDER BY ProductName", conn)
    ...
    Private Sub LoadCurrentStock()
        Try
            ...
            Using conn As New SqlConnection(GetConnectionString())
                conn.Open()
                Dim cmd As New SqlCommand("SELECT CurrentStock FROM RetailInventory WHERE ProductID = @ProductID", conn)
                ...
                Using trans As SqlTransaction = conn.BeginTransaction()
                    Try
                        ...
                        Select Case adjustmentType
                            Case "Increase"
                                updateSql = "UPDATE RetailInventory SET CurrentStock = CurrentStock + @Quantity WHERE ProductID = @ProductID"
                            Case "Decrease"
                                updateSql = "UPDATE RetailInventory SET CurrentStock = CurrentStock - @Quantity WHERE ProductID = @ProductID"
                            Case "Count Adjustment"
                                updateSql = "UPDATE RetailInventory SET CurrentStock = @Quantity WHERE ProductID = @ProductID"
                        End Select
