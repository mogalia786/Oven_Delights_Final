...
Public Partial Class ProductUpsertForm
    ...
    Private Function EnsureProduct(conn As Microsoft.Data.SqlClient.SqlConnection, tx As Microsoft.Data.SqlClient.SqlTransaction) As Integer
        ' Try find by SKU; else insert
        Using cmdFind As New Microsoft.Data.SqlClient.SqlCommand("SELECT ProductID FROM dbo.Retail_Product WHERE SKU=@sku", conn, tx)
            ...
            If o IsNot Nothing AndAlso o IsNot DBNull.Value Then
                Dim pid = CInt(o)
                Using cmdUpd As New Microsoft.Data.SqlClient.SqlCommand("UPDATE dbo.Retail_Product SET Name=@n, Category=@c, Subcategory=@sc, Description=@d, UpdatedAt=SYSUTCDATETIME() WHERE ProductID=@id", conn, tx)
                    cmdUpd.Parameters.AddWithValue("@n", txtName.Text.Trim())
                    cmdUpd.Parameters.AddWithValue("@c", categorySelector.SelectedCategoryName)
                    cmdUpd.Parameters.AddWithValue("@sc", categorySelector.SelectedSubcategoryName)
                    cmdUpd.Parameters.AddWithValue("@d", If(String.IsNullOrWhiteSpace(txtDescription.Text), CType(DBNull.Value, Object), txtDescription.Text.Trim()))
            ...
            End If
        End Using
        Using cmdIns As New Microsoft.Data.SqlClient.SqlCommand("INSERT INTO dbo.Retail_Product(SKU, Name, Category, Subcategory, Description) VALUES(@sku,@n,@c,@sc,@d); SELECT CAST(SCOPE_IDENTITY() AS INT);", conn, tx)
            cmdIns.Parameters.AddWithValue("@sku", txtSKU.Text.Trim())
            cmdIns.Parameters.AddWithValue("@n", txtName.Text.Trim())
            cmdIns.Parameters.AddWithValue("@c", categorySelector.SelectedCategoryName)
            cmdIns.Parameters.AddWithValue("@sc", categorySelector.SelectedSubcategoryName)
            cmdIns.Parameters.AddWithValue("@d", If(String.IsNullOrWhiteSpace(txtDescription.Text), CType(DBNull.Value, Object), txtDescription.Text.Trim()))
