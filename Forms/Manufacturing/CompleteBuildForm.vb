    ...
    Public Class CompleteBuildForm
        ...
        Private Sub OnCompleted(sender As Object, e As EventArgs)
            ...
            Try
                ...
                Using cn As New SqlConnection(cs)
                    cn.Open()
                    Using cmd As New SqlCommand("IF COL_LENGTH('dbo.InternalOrderHeader','CompletedDate') IS NOT NULL BEGIN UPDATE dbo.InternalOrderHeader SET Status='Completed', CompletedDate = SYSUTCDATETIME() WHERE InternalOrderID = @id END ELSE BEGIN UPDATE dbo.InternalOrderHeader SET Status='Completed' WHERE InternalOrderID = @id END", cn)
                        cmd.Parameters.AddWithValue("@id", currentIOId)
                        cmd.ExecuteNonQuery()
                    End Using
                    ' Also mark BomTaskStatus as Completed so it disappears from Pending dashboards
                    Using cmd2 As New SqlCommand("UPDATE dbo.BomTaskStatus SET Status = N'Completed', UpdatedAtUtc = SYSUTCDATETIME() WHERE InternalOrderID = @id", cn)
