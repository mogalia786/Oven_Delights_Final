...
Namespace Manufacturing
    ...
    Public Class BuildProductForm
        ...
        Private Sub SaveProductRecipe()
            ' Save product to BOTH Manufacturing_Product AND Retail_Product tables for full system compatibility
                ...
                Using trans As SqlTransaction = conn.BeginTransaction()
                    Try
                        ...
                        Using cmd As New SqlCommand(sql1, conn, trans)
                            ...
                            cmd.Parameters.AddWithValue("@cat", categorySelector.SelectedCategoryName)
                        ...
                        Using cmd As New SqlCommand(sql2, conn, trans)
                            cmd.Parameters.AddWithValue("@sku", If(String.IsNullOrEmpty(skuValue), DBNull.Value, CObj(skuValue)))
                            cmd.Parameters.AddWithValue("@name", productName)
                        ...
                        Using cmd As New SqlCommand(sql3, conn, trans)
                            cmd.Parameters.AddWithValue("@pid", retailProductId)
                            variantId = Convert.ToInt32(cmd.ExecuteScalar())
                        End Using

                        ' 4. Save recipe structure to RecipeNode table (linked to manufacturing product)
                        SaveRecipeNodes(manufacturingProductId, conn, trans)
