...
Public Class CreateSubRecipeForm
    ...
    Private Sub LoadIngredients()
        ...
        Using conn As New SqlConnection(_connectionString)
            ...
            Using cmd As New SqlCommand(query, conn)
                ...
                Using reader As SqlDataReader = cmd.ExecuteReader()
                    While reader.Read()
                        cboIngredient.Items.Add(New With {
                            .ProductID = reader.GetInt32(0),
                            .Name = reader.GetString(1)
                        })
                    End While
    ...
    Private Sub cboSubRecipe_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboSubRecipe.SelectedIndexChanged
        If cboSubRecipe.SelectedItem Is Nothing Then Return

        _selectedSubRecipeID = CType(cboSubRecipe.SelectedItem, Object).ProductID
    ...
    Private Sub btnAddIngredient_Click(sender As Object, e As EventArgs) Handles btnAddIngredient.Click
        ...
        If Not IsNumeric(txtPackageSize.Text) OrElse CDec(txtPackageSize.Text) <= 0 Then
            MessageBox.Show("Please enter a valid package size.", "Validation", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If

        Dim ingredientID As Integer = CType(cboIngredient.SelectedItem, Object).ProductID
        Dim ingredientName As String = CType(cboIngredient.SelectedItem, Object).Name
        Dim quantity As Decimal = CDec(txtQuantity.Text)
        Dim unit As String = cboUnit.Text
        Dim packageSize As Decimal = CDec(txtPackageSize.Text)
    ...
    Private Sub CalculateTotalCost()
        ...
        lblTotalCost.Text = $"Total Cost Per Sub-Recipe: {total.ToString("C2")}"
