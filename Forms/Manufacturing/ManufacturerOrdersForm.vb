    ...
    Private Sub LoadOrders()
        Try
            Using conn As New SqlConnection(_connString)
                ...
                ' Filter by order type (Cake or Order)
                If _orderType = "Cake" Then
                    sql &= " AND o.OrderType = 'Cake'"
                ElseIf _orderType = "General" Then
                    sql &= " AND o.OrderType = 'Order'"
                End If
            ...
            Using conn As New SqlConnection(_connString)
                ...
                Using reader = cmdOrder.ExecuteReader()
                    If reader.Read() Then
                        ...
                        isCakeOrder = (orderType = "Cake")
                        
                        details.AppendLine("═══════════════════════════════════════")
                        details.AppendLine($"ORDER: {orderNumber}")
                        details.AppendLine($"Type: {If(isCakeOrder, "CUSTOM CAKE", "GENERAL ORDER")}")
