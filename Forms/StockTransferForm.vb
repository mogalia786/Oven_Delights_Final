...
Public Class StockTransferForm
    ...
    Private Sub InitializeCustomControls()
        ...
        ' Header panel
        Dim headerPanel As New Panel With {.Dock = DockStyle.Top, .Height = 200, .Padding = New Padding(12)}
        ...
        btnCreateTransfer = New Button With {.Text = "Create Transfer", .Top = 148, .Left = 550, .Width = 150, .Height = 30}
        ...
        ' Grid
        dgvTransfers = New DataGridView With {.Dock = DockStyle.Fill, .AllowUserToAddRows = False, .AllowUserToDeleteRows = False, .ReadOnly = True, .AutoGenerateColumns = True}
        ...
        Using con As New SqlConnection(connectionString)
            ...
            Using tx = con.BeginTransaction()
                Try
                    ...
                    Using cmd As New SqlCommand("INSERT INTO InterBranchTransfers (TransferNumber, FromBranchID, ToBranchID, ProductID, Quantity, UnitCost, TotalValue, TransferDate, Reference, Status, CreatedBy, CreatedDate) OUTPUT INSERTED.TransferID VALUES (@num, @from, @to, @prod, @qty, @cost, @total, @date, @ref, 'Pending', @user, GETDATE())", con, tx)
    ...
    Private Function GetProductCost(productId As Integer, branchId As Integer, con As SqlConnection, tx As SqlTransaction) As Decimal
        Using cmd As New SqlCommand("SELECT TOP 1 ISNULL(AverageCost, 0) FROM Retail_Stock WHERE VariantID = @prod AND BranchID = @branch", con, tx)
            cmd.Parameters.AddWithValue("@prod", productId)
    ...
    Private Sub UpdateRetailStock(productId As Integer, branchId As Integer, qtyDelta As Decimal, con As SqlConnection, tx As SqlTransaction)
        ...
        Using cmdCheck As New SqlCommand("SELECT COUNT(*) FROM Retail_Stock WHERE VariantID = @prod AND BranchID = @branch", con, tx)
            ...
            If exists > 0 Then
                Using cmdUpdate As New SqlCommand("UPDATE Retail_Stock SET QtyOnHand = QtyOnHand + @delta WHERE VariantID = @prod AND BranchID = @branch", con, tx)
                ...
                End Using
            Else
                Using cmdInsert As New SqlCommand("INSERT INTO Retail_Stock (VariantID, BranchID, QtyOnHand, AverageCost) VALUES (@prod, @branch, @qty, 0)", con, tx)
